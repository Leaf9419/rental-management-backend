...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        // 發送Email驗證碼
        verificationService.sendEmailVerificationCode(saved.getEmail(), saved.getUserId());
    ...
    public void verifyEmail(String code) {
        ...
        if (user.getEmailVerified()) {
            throw new BusinessException("Email已經驗證過了");
        }

        // 驗證驗證碼
        verificationService.verifyEmailCode(user.getEmail(), code, user.getUserId());
    ...
    public void resendEmailVerificationCode() {
        ...
        verificationService.sendEmailVerificationCode(user.getEmail(), user.getUserId());
    }
 ...
 * SecurityContextHolder.getContext().setAuthentication(authentication);
 * 
 * Spring Security不管是登錄、JWT、自訂filter，一律都使用Authentication物件
